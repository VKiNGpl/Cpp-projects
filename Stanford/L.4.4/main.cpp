 ...
 * they are served and occupy the casher for a randomly
 * chosen period of time. After the service time is complete,
 * the cashier is free to serve the next customer in line.
 *
 ...
 *
 * 2. If the cashier is busy, note that the ashier has
...
#include "random.h"     // Stanford lib
#include "queue.h"      // Stanford lib
 ...
 * Then, if the cashier is busy (indicated by nonzero value for
 * serviceTimeRemaining), the program decrements the variable to
 * indicate that one more time unit has passed. Finally, if the
 * casher is free, the simulation serves another customer from
